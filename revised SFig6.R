...
FFU_stock <-  readxl::read_xlsx("Source Data File.xlsx", sheet = "Supplementary Fig6")
...
dissemination_rate <- Dataset_final %>%
  ...
  summarise(
    infected    = sum(Leg_Dissemination, na.rm = TRUE),
    sample_size = n(),
    .groups     = "drop"
  )


# combined_dissemination_rate_stock <- dissemination_rate %>%
...
nrow(combined_dissemination_rate_stock)
length(unique(combined_dissemination_rate_stock$Virus))

# ============================================================
# PREPARE VARIABLES
# ============================================================
combined_dissemination_rate_stock$Mosquito_strain <- factor(
  combined_dissemination_rate_stock$Mosquito_strain,
  levels = c("WT", "wAlbB", "wMel")
)

combined_dissemination_rate_stock <- combined_dissemination_rate_stock %>%
  mutate(Mosquito_strain = recode(Mosquito_strain, "wMel" = "wMelM"))



# combined_dissemination_rate_stock$log10_titre <-
#   log10(combined_dissemination_rate_stock$`GE/uL_stock`)

combined_dissemination_rate_stock$log10_titre <-
  log10(combined_dissemination_rate_stock$`Final FFU/mL`)

titre_mean <- mean(combined_dissemination_rate_stock$log10_titre, na.rm = TRUE)
titre_sd   <- sd(combined_dissemination_rate_stock$log10_titre,   na.rm = TRUE)

combined_dissemination_rate_stock$log10_titre_scaled <-
  as.numeric(scale(combined_dissemination_rate_stock$log10_titre))

combined_dissemination_rate_stock$Serotype_label <-
  paste0("DENV-", combined_dissemination_rate_stock$Serotype)

combined_dissemination_rate_stock$Serotype <- factor(
  combined_dissemination_rate_stock$Serotype)
...
# ============================================================
# REVISED FIGURE A
# Predicted dissemination by titre, strain, serotype
...
Fig_revised_A <- ggplot() +
  ...
  scale_y_continuous(
    name   = "Dissemination rate (%)",
    breaks = c(0, 25, 50, 75, 100),
    limits = c(0, 100)
  ) +
  scale_x_continuous(
    #name   = expression(paste("Input stock virus GE/µL (log"[10], ")")),
    name   = expression(paste("Input stock virus FFU/ml (log"[10], ")")),
    breaks = seq(2, 5, by = 1)
  ) +
  guides(color = "none") +
  theme_classic() +
...
serotype_lookup <- unique(
  combined_dissemination_rate_stock[, c("Virus", "Serotype_label")]
)
# --- Add serotype info ---
